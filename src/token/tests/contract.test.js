...
describe('SavingToken', () => {
    ...
    it('executes transfer function, sends tokens from accounts[0] to accounts[1]', async () => {
        ...
        var balanceowner = await savingToken.methods.balances(owner).call();
        console.log(await savingToken.methods.nextTxTime(accounts[0]).call());

        console.log("Acc0unt : " ,accounts[1] , balanceAcc1);
        console.log("Acc0unt : " ,accounts[2] , balanceAcc2);
    ...
    it('is Should change the LockTime of an account', async () => {
        //changing LockTime to 1 minute (60 seconds)
